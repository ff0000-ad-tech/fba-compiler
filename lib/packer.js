...
const log = debug('fc:lib:packer')
const log1 = debug('fc:lib:packer:+')
debug.disable('fc:lib:packer:+')
...
const packAsset = async (assetPath, chunkType) => {
	...
	const chunkSize = assetBytes.length
	...
	const chunkBytesLength = chunkTypeBA.length + assetBytes.length
	const chunkBytes = Buffer.concat([chunkTypeBA, assetBytes], chunkBytesLength)
	...
	const fullChunkBA = Buffer.concat(
		[chunkSizeBA, chunkTypeBA, assetBytes, crcBA],
		chunkSizeBA.length + chunkTypeBA.length + assetBytes.length + crcBA.length
	)
