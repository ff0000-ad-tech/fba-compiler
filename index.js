...
const packfbAc = () => {		
	...
	const tmpBytes = Buffer.concat(
		[chunkTypeBA, chunkCountBA], 
		chunkTypeBA.length + chunkCountBA.length
	)
	...
	return Buffer.concat(
		[sizeBA, tmpBytes, crcBA],
		sizeBA.length + tmpBytes.length + crcBA.length
	)	
}
...
const buildPng = () => {
	...
	// IDAT
	var filteredData = packer.filterData(
		new Buffer(4 * imageDimension * imageDimension), 
		imageDimension, 
		imageDimension
	)
	// compress it
